...
`atelet` runs its container with `privileged: true`, so a cluster that
enforces Pod Security Admission on this namespace needs
`pod-security.kubernetes.io/enforce: privileged` on it. When
`createNamespace` is `true`, set that label through `namespaceLabels`; when
the namespace already exists (the default), set it on that namespace by
hand.

Actor sandbox pods run in each `WorkerPool`'s own namespace, which this chart
does not create, and they are not privileged, so this label is only needed
for the release namespace.
...
| Key | Default | Notes |
|-----|---------|-------|
| `createNamespace` | `false` | Have the chart create the release namespace |
| `namespaceLabels` | `{}` | Labels applied to the namespace when `createNamespace` is `true`; set `pod-security.kubernetes.io/enforce: privileged` on clusters enforcing Pod Security Admission, since atelet runs privileged |
